...
- The backend is fully built, secured with JWT authentication, and contains CRUD routes for all 5 collections plus integrations (`/api/trips`, `/api/destinations`, `/api/places`, `/api/events`, `/api/upload`, `/api/admin/stats`).
...
- `backend/.env`: Needs `MONGO_URI`, `PORT=5000`, `JWT_SECRET`, and `DROPBOX_ACCESS_TOKEN`.
